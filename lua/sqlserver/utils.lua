...
local function remove_lsp_nulls(value)
...
---makes a request to the SQL Server lsp client
...
return {
  contains = contains,
  remove_lsp_nulls = remove_lsp_nulls,
  ...
  get_query_result_async = function(query_result_summary)
    ...
    local columnNames = vim
      .iter(query_result_summary.batchSummaries[1].resultSetSummaries[1].columnInfo)
      :map(function(ci)
