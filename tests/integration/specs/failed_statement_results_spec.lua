...
T["Failed statements should not create empty result buffers"] = require("tests.helpers").async(function()
...
WHERE Person.ID = 2;
  ...
  assert(#summaries == 2, "Expected SQL Tools Service to report both result-set positions: " .. vim.inspect(summaries))
  ...
  assert(vim.api.nvim_buf_get_name(buffers[1]):match("/results 2%.sqlresult$"), "The result ordinal was not preserved")
