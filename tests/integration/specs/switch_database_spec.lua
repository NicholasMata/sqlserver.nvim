...
local switch_db_async = function()
  ...
  local success = false
  sqlserver.switch_database(function()
    success = true
  ...
  coroutine.yield()
  if not success then
    error("sqlserver.switch_database did not resume the callback within 1 minute", 0)
  end
end
...
T["Switch database should work"] = require("tests.helpers").async(function()
