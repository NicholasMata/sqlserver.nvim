...
T["Result column navigation follows rendered cell boundaries"] = require("tests.helpers").async(function()
  ...
  vim.api.nvim_win_set_cursor(0, { 3, 0 })
  vim.cmd("normal 2]c")
  assert(vim.api.nvim_win_get_cursor(0)[2] == 16, "Column mappings should accept Vim counts")
  vim.cmd("normal 2[c")
  assert(vim.api.nvim_win_get_cursor(0)[2] == 0)
  ...
  vim.cmd("normal l")
  assert(vim.api.nvim_win_get_cursor(0)[2] == 1, "Visual l should move one character")
  vim.cmd("normal ]c")
  assert(vim.api.nvim_win_get_cursor(0)[2] == 7, "Visual ]c should jump to the next column")
  vim.cmd("normal [c")
  assert(vim.api.nvim_win_get_cursor(0)[2] == 0, "Visual [c should jump to the previous column")
  vim.cmd("normal ]c")
  vim.cmd("normal j")
  assert(vim.api.nvim_win_get_cursor(0)[1] == 4, "Visual j should move one rendered line")
