...
T["SQL Tools Service releases should be explicit and validated"] = require("tests.helpers").async(function()
  local release = downloader.get_release("1.2.3", "Linux", "x64")
  assert(release.version == "1.2.3")
  assert(release.filename == "Microsoft.SqlTools.ServiceLayer-linux-x64-net10.0.tar.gz")
  ...
  local function make_archive(name, executable_contents)
    local source_dir = vim.fs.joinpath(test_dir, name)
    local archive = vim.fs.joinpath(test_dir, name .. ".tar.gz")
    vim.fn.mkdir(source_dir, "p")
    vim.fn.writefile(executable_contents, vim.fs.joinpath(source_dir, "MicrosoftSqlToolsServiceLayer"))
  ...
  local fresh_archive = make_archive("fresh", { "#!/bin/sh", "echo fresh" })
  local installed, install_error = downloader.download_tools_async({ url = "file://" .. fresh_archive }, test_dir)
  assert(installed, install_error)
  local executable = vim.fs.joinpath(test_dir, "sqltools", "MicrosoftSqlToolsServiceLayer")
  assert(vim.fn.executable(executable) == 1, "A fresh managed install should be executable")
  assert(vim.fn.readfile(executable)[2] == "echo fresh")

  local upgrade_archive = make_archive("upgrade", { "#!/bin/sh", "echo upgraded" })
  installed, install_error = downloader.download_tools_async({ url = "file://" .. upgrade_archive }, test_dir)
  assert(installed, install_error)
  assert(vim.fn.readfile(executable)[2] == "echo upgraded", "An upgrade should replace the managed install")
  assert(#vim.fn.glob(vim.fs.joinpath(test_dir, "sqltools.previous-*"), false, true) == 0)
  ...
  installed, install_error = downloader.download_tools_async({ url = "file://" .. archive }, test_dir)
