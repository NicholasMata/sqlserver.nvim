...
T["Query selection should describe statement and buffer scopes"] = require("tests.helpers").async(function()
  ...
  assert(buffer.range.endLine == 1 and buffer.range.endColumn == 9)
  ...
  assert(visual.range.endLine == 0 and visual.range.endColumn == 6)
  ...
  assert(utf16_visual.range.startColumn == 2 and utf16_visual.range.endColumn == 8)
  ...
  assert(line_visual.range.endLine == 1 and line_visual.range.endColumn == 18)
