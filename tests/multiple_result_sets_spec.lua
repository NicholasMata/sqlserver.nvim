...
local function find_results_buffers()
  local buffers = {}
  for _, bufnr in ipairs(vim.api.nvim_list_bufs()) do
    if vim.api.nvim_buf_is_valid(bufnr) and vim.api.nvim_buf_get_name(bufnr):match("/results %d+%.sqlresult$") then
...
SELECT * FROM TestDbA.dbo.Person WHERE ID = 2;
    ...
    local buffers = find_results_buffers()
